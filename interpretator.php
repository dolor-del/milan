...
//Описание констант
       ...
       define("_INCREMENT_",   		 29);   /*  инкремент ++  */
       ...
       function ct_AddElement($Key_Word, $Code_Key_Word) {
           ...
           return $ct_Element;
             ...
             $Table_Key_Words[12]= ct_AddElement('ENDREPEAT',   _ENDREPEAT_);
             $Table_Key_Words[13]= ct_AddElement('REPEAT',    	_REPEAT_);
             $Table_Key_Words[14]= ct_AddElement('UNTIL',    	_UNTIL_);
             $Table_Key_Words[15]= ct_AddElement('ENDCASE',    	_ENDCASE_);
             $Table_Key_Words[16]= ct_AddElement('CASE',    	_CASE_);
             $Table_Key_Words[17]= ct_AddElement('OF',    	_OF_);
             $Table_Key_Words[18]= ct_AddElement('OR',    	_OR_);
             $Table_Key_Words[19]= ct_AddElement('DEFAULT',    	_DEFAULT_);
	   ...
	   /*    34      */   'Конструкция <оператор>. Неверный оператор REPEAT.',
	   /*    35      */   'Несоответствие в операторах REPEAT-UNTIL-ENDREPEAT.',
	   /*    36      */   'Переполнение стека Stek_rep.',
	   /*    37      */   'Нехватка элементов в стеке Stek_rep.',
	   /*    38      */   'Конструкция <оператор>. Неверный оператор CASE.',
	   /*    39      */   'Несоответствие в операторах CASE-OF-OR-DEFAULT-ENDCASE.',
	   /*    40      */   'Переполнение стека Stek_case.',
	   /*    41      */   'Нехватка элементов в стеке Stek_case.',
	   /*    42      */   'Переполнение стека StekCs.',
	   /*    43      */   'Нехватка элементов в стеке StekCs.'];
 ...
 function Print_Error($Variant) {
    ...
    switch ($Variant) {
                Case 1 :
                   echo ('<html lang="ru"><head><title>РЕЗУЛЬТАТ РАБОТЫ ЛЕКСИЧЕСКОГО АНАЛИЗАТОРА</title></head><body><br><H2 style="text-align:center">РЕЗУЛЬТАТ РАБОТЫ ЛЕКСИЧЕСКОГО АНАЛИЗАТОРА</H2><br><hr><br>
                   <table style="text-align:center; padding:3px; border-spacing:3px"><tr><td >НАЙДЕНА ОШИБКА: </td><td style="text-align:center"><span style="color:red"> ' . $Error_Message[$Code_Error] . '</span></td></tr>
                   <tr><td> СТРОКА: </td><td style="text-align:left"><span style="color:blue">' . $Number_String . '</span></td></tr>
                   <tr><td> ПОЗИЦИЯ:</td><td style="text-align:left"><span style="color:blue">' . $Position . '</span></td></tr></table>
                   <br><hr><br><div style="text-align:center">
                ...
                Case 2 :
                   echo ('<html lang="ru"><head><title>РЕЗУЛЬТАТ РАБОТЫ СИНТАКСИЧЕСКОГО АНАЛИЗАТОРА</title></head><body><br><H2 style="text-align:center">РЕЗУЛЬТАТ РАБОТЫ СИНТАКСИЧЕСКОГО АНАЛИЗАТОРА</H2><br><hr><br>
                   <table style="text-align:center; padding:3px; border-spacing:3px"><tr><td >НАЙДЕНА ОШИБКА: </td><td style="text-align:center"><span style="color:red"> ' . $Error_Message[$Code_Error] . '</span></td></tr>
                   <tr><td> ЛЕКСЕМА: </td><td style="text-align:left"><span style="color:blue">' . $Number_Lexem . '</span></td></tr></table>
                   <br><hr><br><div style="text-align:center">
                   <form method="post" action="INTERFACE.php">
                   <input type="submit" value="    ОК    ">
                   <input name="source" type="hidden" value="'.$ct_buf.'">    
 ...
 /************************************************************************/
  ...
  Function Isdigit($Figure) {
    $ct_res=preg_match("/[0-9]/ui", $Figure);
    return $ct_res;
  } /* End Isdigit */
  ...
  Function Isalpha($Letter) {
   ...
   $ct_res=preg_match("/[A-Za-zА-Яа-я]/ui", $Letter);
   return $ct_res;
  } /* End Isalpha */
...
function Lexical_Analyzer() {
	...
	function End_Lexical_Analyzer()
	{
		...
		if($ct_milanMode==0)
		{
			...
			echo "<br><hr><br><div style=\"text-align:center\"><span style=\"color:blue\"> НОРМАЛЬНОЕ ЗАВЕРШЕНИЕ РАБОТЫ ЛЕКСИЧЕСКОГО АНАЛИЗАТОРА</span><br><br>
  <form  id=\"sourceform\" name=\"sourceform\" action='MILAN.PHP' method=\"post\">
    ...
    <input name=\"tab_ident\" type=\"hidden\" value='" .implode('%',$Tab_Identifiers). "'>       <!--поле хранит данные таблицы идентификаторов -->
    <input name=\"tab_const\" type=\"hidden\" value='" .implode('%',$Tab_Constants). "'>         <!--поле хранит данные таблицы констант -->
  <input name=\"next\" type=\"submit\" value=\"Далее\" style=\"text-align:center\" size=\"40\">
  <input name=\"mode\" type=\"hidden\" value=\"1\">                                          <!--поле хранит текущий режим работы скрипта -->
	...
	Function Letter() {
		...
		while ((Isalpha($Input_Letter)) || (Isdigit($Input_Letter))) {
			...
			/* функция у4: накопление символов ключевого слова или идентификатора */ $Word = $Word.$Input_Letter;
	...
	/*                               ENDREPEAT-->REPEAT+1, ELSE-->ENDIF+1   */
	...
	function Setup_Refference()
	{
		...
		Stek_Integer(0,$Stek_rep,$Top_rep,$r);
		...
		do
		{
			switch($Tab_Lexems[$Number_Lexem]->Code):
				...
				Case   _REPEAT_ :
					...
					/* функция y6: значение $Number_Lexem занести   */
					/* в стек $Stek_rep ($Number_Lexem-->$Stek_rep) */
					if (!Stek_Integer(2,$Stek_rep,$Top_rep,$Number_Lexem))
					{
						/* Переполнение стека REP */
						$Code_Error=35;
						return 0;
					}
				...
				Case  _ENDREPEAT_ :
					...
					/* функция y7: Снять вершину стека $Stek_rep в переменную
					$r ($r←$Stek_rep). Лексеме с номером $Number_Lexem
					присвоить значение $r
					($Tab_Lexems[$Number_Lexem]->Value=$r+1) [ENDREPEAT→REPEAT+1] */

					if (!Stek_Integer(1,$Stek_rep,$Top_rep,$r))
					{
						/* Нехватка элементов в стеке REP */
						$Code_Error=36;
						return 0;
					}
				...
				Case _CASE_ :
					...
					/* прочитать очередную лексему с номером  $Number_Lexem */
					$Number_Lexem++;
					while ($Tab_Lexems[$Number_Lexem]->Code != 30) {
						/* функция y11: $Number_Lexem++, */
						/* прочитать очередную лексему с номером  $Number_Lexem */
						$Number_Lexem++;
						...
						if ($Number_Lexem>$Number_Lexems_Programm)
						{
							/* Несоответствие  операторах CASE-OF-OR-DEFAULT-ENDCASE */
							$Code_Error=38;
							return 0;
						}
					}

					/* функция y8: значение $Number_Lexem занести в  */
					/* стек $Stek_case ($Number_Lexem-->$Stek_case)  */
				...
				Case 30 :
					...
					/*  функция y9: Снять вершину стека $Stek_case в переменную $r
					($r←$Stek_case). Снять вершину стека $Stek_case в
					переменную $s ($s ← $Stek_case). Значение $r присвоить
					лексеме с номером $s ($Tab_Lexems[$s]->Value=$r) [: →OR, : →DEFAULT].
					Значение $Number_Lexem занести в стек $Stek_case ($Number_Lexem→$Stek_case);   */
					...
					if (!Stek_Integer(1,$Stek_case,$Top_case,$s))
					{
						/* Нехватка элементов в стеке CASE */
						$Code_Error=40;
						return 0;
				...
				Case _ENDCASE_ :
					...
					/*  функция y10: Снять вершину стека $Stek_case в переменную
					$r ($r←$Stek_case). Присвоить значение $Number_Lexem
					лексеме с номером $r ($Tab_Lexems[$r]->Value=$Number_Lexem)[: → ENDCASE];  */
					...
					/* функция y10: значение $Number_Lexem занести в  */
					/* лексему $Tab_Lexems[$r]  */
	...
	Function Print_Tab_Lexems()
	{
		...
		global $ct_TabLEX_Code_str;
		global $ct_TabLEX_Value_str;
		echo '<html lang="ru"><head><title>ЛЕКСИЧЕСКИЙ АНАЛИЗАТОР</title><link rel="stylesheet" href="styles.css"></head><br><H2> <div style="text-align:center">РЕЗУЛЬТАТ РАБОТЫ ЛЕКСИЧЕСКОГО АНАЛИЗАТОРА</div></H2><br><hr><br>';
		/* Начало процедуры Print_Tab_Lexems */
		/* Формирование шапки таблицы массива лексем */
		$Zastav  = '<div><div class="border" style="width:700px"><table class="mytable"><tr><td colspan=19><div>Т А Б Л И Ц А &nbsp; Л Е К С Е М</div></td></tr>';
		...
		for ($i=1; $i<=$End;$i++)
		{
			...
			for ($j=0;$j<=9;$j++)
			{
				...
				else
				{
					...
					$TabLex .='<td>' . Space(2-strlen($Number)) . $Number . ',' . Space(2-strlen($Number_)) . $Number_ . '</td>';
				}
			}
			$Zastav  .= $TabLex .'</tr>';
		}
	...
	Function Print_Tab_Identifiers_Constants()
	{
		...
		$Tab_Identifiers_Constants  = '<br><br><div class="border" style="width: 400px;"><table class="mytable"><tr><td colspan=2>МАССИВ ИДЕНТИФИКАТОРОВ&nbsp;</td></tr>';
		$Tab_Identifiers_Constants  .= '<tr><td> Номер </td><td> ИДЕНТИФИКАТОР </td></tr>';

		for($i=1;$i<=$Number_Identifiers;$i++)
		{
			$Tab_Identifiers_Constants .=  '<tr><td>' . $i . '</td><td>' . $Tab_Identifiers[$i] . '</td></tr>';

		}

		/* Формирование таблицы массива констант */
		$Tab_Identifiers_Constants  .= '</table></div><br><br><div class="border" style="width:300px"><table class="mytable"><tr><td colspan=2>МАССИВ КОНСТАНТ&nbsp;</td></tr>';
		$Tab_Identifiers_Constants  .= '<tr><td> Номер </td><td> КОНСТАНТА </td></tr>';
		for($i=1;$i<=$Number_Constants;$i++)
		{
			$Tab_Identifiers_Constants .=  '<tr><td>' . $i . '</td><td>' . $Tab_Constants[$i] . '</td></tr>';
		}

		$Tab_Identifiers_Constants  .= '</table></div></div>';
	...
	$Position=0;
	$Number_String=1;
	$ct_spec= '/['.Chr(9). Chr(10).Chr(13). Chr(32).']/ui';

	do
	{
		/* Игнорирование спец. символов и пробела */
		while (preg_match($ct_spec,$Input_Letter))
		{
			switch (ord($Input_Letter)):
		...
		if(isAlpha($Input_Letter))      // если введённый символ является буквой
			Letter();
		elseif(isDigit($Input_Letter))  // если введённый символ является цифрой
			Digit();
		else {
			switch ($Input_Letter):
  ...
  Function ProcedureP()
 {
  ...
  switch ($Tab_Lexems[$Number_Lexem]->Code):
     Case _IDENTIFIER_ :
                   ...
                   if ($Tab_Lexems[$Number_Lexem]->Code == _INCREMENT_) {
      ...
      Case _READ_ :
                   ...
                   echo ('<script type="text/javascript">
                            ...
                            <input type="text" name="stdin">                                               <!--в поле с именем "stdin" хранится введённое пользователем число -->
                              ...
                              <input name="tab_ident" type="hidden" value="' .implode('%',$Tab_Identifiers). '">       <!--поле хранит данные таблицы идентификаторов -->
                              <input name="tab_const" type="hidden" value="' .implode('%',$Tab_Constants). '">         <!--поле хранит данные таблицы констант --> 
                              <input name="readbuf" type="hidden" value="'. implode("%",$ct_readBUF) .'">      <!--функция implode("%",$ct_readBUF) преобразует массив данных в строку разделённую "%" -->
  ...
  Function ProcedureT()
{
 ...
 function ProcedureV() {
	...
	if ($Tab_Lexems[$Number_Lexem]->Code == _OR_) {
		ProcedureV();
	}
 ...
 function ProcedureF() {
	...
	if ($Tab_Lexems[$Number_Lexem]->Code == _OR_) {
		...
		if ($Tab_Lexems[$Number_Lexem]->Code == _CONSTANT_) {
			...
			while ($Tab_Lexems[$Number_Lexem+1]->Code == 32) {

				$NomIdent=$Tab_Lexems[$Number_Lexem]->Value;
				$temp[$j] = $Tab_Constants[$NomIdent];
				$j++;
				$Number_Lexem += 2;
			...
			if (!Stek_Integer(1, $StekCs, $TopCs,$Bi)) {
				/* Нехватка элементов в стеке $SteCs */
				$Code_Error=42;
				return 0;
			}

			if (!Stek_Integer(2, $StekCs, $TopCs,$Bi)) {
				/* Нехватка элементов в стеке $SteCs */
				$Code_Error=42;
				return 0;
			}
			...
			if ($result == $Bi) {
				...
				if ($Tab_Lexems[$Number_Lexem]->Code == 30) {

					$Number_Lexem++;

					ProcedureL();

					if (!Stek_Integer(1, $StekCs, $TopCs,$Ai))
					{
						/* Нехватка элементов в стеке $SteCs */
						$Code_Error=42;
						return 0;
					...
					if (!Stek_Integer(2, $StekCs, $TopCs, $t))
					{
						/* Нехватка элементов в стеке $SteCs */
						$Code_Error=42;
						return 0;
					...
					if (!Stek_Integer(2, $StekCs, $TopCs, $Ai))
					{
						/* Нехватка элементов в стеке $SteCs */
						$Code_Error=42;
						return 0;
					...
					/* Несоответствие в операторах CASE-OF-OR-DEFAULT-ENDCASE */
					$Code_Error=38;
					return 0;
				}
			} else {
  ...
  switch ($Tab_Lexems[$Number_Lexem]->Code):
   ...
   case _CASE_:
				    ...
				    $Number_Lexem++;
   					...
   					if($Tab_Lexems[$Number_Lexem]->Code == _OF_) {
						...
						if ($Tab_Lexems[$Number_Lexem]->Code == _ENDCASE_) {

							$t = 0;

							if (!Stek_Integer(1, $StekCs, $TopCs,$t))
							{
								/* Нехватка элементов в стеке $SteCs */
								$Code_Error=42;
								return 0;
							...
							/* Конструкция <оператор>. Неверный оператор CASE */
							$Code_Error=37;
							return 0;
						}
					}  else {
   ...
   case _IDENTIFIER_ :
                    ...
                    } elseif ($Tab_Lexems[$Number_Lexem]->Code == _INCREMENT_) {
						$NomIdent=$Tab_Lexems[$Number_Lexem]->Value;
						if (!Stek_Integer(2, $StekRes, $TopRes, $ArrIdent[$NomIdent]))
						{
							/* Переполнение стека $StekRes */
   ...
   case _REPEAT_ :
				...
				while (TRUE)
				{
				 ...
				 if ($Tab_Lexems[$Number_Lexem]->Code==_UNTIL_) {
				  ...
				  /* проверка истинности условия: $Ai==1 (TRUE), */
				  /* $Ai==0 (FALSE)                              */
				  if ($Ai==0) /* обработка TRUE */
				  {
				   /* функция y4: чтение следующей лексемы ($Number_Lexem++) */

				   if ($Tab_Lexems[$Number_Lexem]->Code==_ENDREPEAT_) {
					/* y17: перейти на лексему номер       */
					/* $Tab_Lexems[$Number_Lexem]->Value      */
					$Number_Lexem=$Tab_Lexems[$Number_Lexem]->Value;
					Continue;
					...
					/* Конструкция <оператор>. Неверный оператор REPEAT. */
					$Code_Error=33;
					return 0;
				   }
				  }
 ...
 /* Начало Синтаксического анализа совмещённого с интерпретацией */
  ...
  Stek_Integer(0, $StekCs,   $TopCs, $Ai);
  ...
  global $ct_buf;
   echo('<br><hr><br><span style = "color: blue">НОРМАЛЬНОЕ ЗАВЕРШЕНИЕ РАБОТЫ ИНТЕРПРЕТАТОРА</span><br><br><form action="INTERFACE.php" method="POST">
                   <input type="submit" value="    ОК    " >
                   <input name="source" type="hidden" value="'.$ct_buf.'">    
                   </form>');
 }